...
var ClassAttack = {
	...
	doAttack: function (unit) {
		...
		if (Attack.getResist(unit, "physical") < 100 && me.getMerc()) {
	...
	checkCorpse: function (unit, revive) {
		...
		if (unit.classid <= 575 && !getBaseStat("monstats2", unit.classid, revive ? "revive" : "corpseSel")) { // monstats2 doesn't contain guest monsters info. sigh..
			return false;
		}
